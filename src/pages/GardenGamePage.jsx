...
import GARDEN_ACTIONS, { TOTAL_FLOWERS } from "@data/garden-game";
...
const STATES = { WELCOME: "welcome", PLAYING: "playing", CORRECT: "correct", WRONG: "wrong", COMPLETED: "completed", CERTIFICATE: "certificate" };
...
export default function GardenGamePage() {
  ...
  const virtueActions = GARDEN_ACTIONS[virtueId] || [];
  ...
  const handleStart = () => { setState(STATES.PLAYING); setCurrentAction(shuffledActions[0]); };

  const handleSelect = (isVirtueAction) => {
    if (state !== STATES.PLAYING || !currentAction) return;
    const answeredCorrectly = (currentAction.isCorrect && isVirtueAction) || (!currentAction.isCorrect && !isVirtueAction);
  ...
  const handleNext = () => {
    if (isDemo && currentIndex + 1 >= (demoMaxQuestions || 3)) { setDemoLimitReached(true); return; }
    if (collectedFlowers.length >= TOTAL_FLOWERS) { setState(STATES.COMPLETED); return; }
    const nextIdx = currentIndex + 1;
    if (nextIdx >= actions.length) { setState(STATES.COMPLETED); }
    else { setCurrentIndex(nextIdx); setCurrentAction(actions[nextIdx]); setState(STATES.PLAYING); }
  };

  const handleRetry = () => { setState(STATES.PLAYING); };

  if (!virtue || !virtueActions.length) return null;
      ...
      <div className="w-100" style={{ maxWidth: 560 }}>
        ...
        {state === STATES.WELCOME && (
          <div className="anim-fade-up text-center mt-5 pt-4">
            ...
            <h1 className="f-display display-6 mb-2">ازرع حديقتك</h1>
            <div className="d-inline-block px-3 py-1 rounded-pill mb-3"
              style={{ background: virtue.colorLight, color: virtueColor }}>
              {virtueIcon} {virtueName}
            </div>
            <p className="fs-5 text-c-light mb-1">مرحباً يا {childName}! 👋</p>
            <p className="text-c-light mb-2">ساعدنا نزرع حديقة {virtueName}! 🌷</p>
            <p className="text-c-light mb-4" style={{ fontSize: "0.9rem" }}>
              سنعرض عليك أفعال مختلفة — اختر الأفعال الصحيحة
              <br />كل فعل صحيح يزرع وردة في حديقتك! 🌺
            </p>
            <button onClick={handleStart} className="btn btn-lg px-5 py-3 fw-bold text-white"
              style={{ background: "linear-gradient(135deg, #43a047, #66bb6a)", border: "none", borderRadius: 16, fontSize: "1.2rem", boxShadow: "0 4px 20px rgba(67,160,71,0.3)" }}>
              🌱 ابدأ الزراعة!
            </button>
          </div>
        )}
          ...
          <>
            <div className="d-flex align-items-center justify-content-between mb-3">
              <div className="d-flex align-items-center gap-2">
                ...
                <span className="f-display fs-6" style={{ color: "#2e7d32" }}>ازرع حديقتك — {virtueName}</span>
              </div>
              <div className="text-c-light f-body small">{currentIndex + 1} / {actions.length}</div>
            </div>

            {/* Garden */}
            <div className="card shadow-sm border-0 p-3 mb-3" style={{ borderRadius: 16 }}>
              <div className="text-center mb-2"><small className="text-c-light">🌷 حديقة {virtueName}</small></div>
              <div className="d-flex flex-wrap justify-content-center gap-1 mb-2" style={{ minHeight: 48 }}>
                {Array.from({ length: TOTAL_FLOWERS }).map((_, i) => (
                  <div key={i} className="d-flex align-items-center justify-content-center rounded-3"
                    style={{ width: 40, height: 40, background: collectedFlowers[i] ? "#e8f5e9" : "#f5f5f5", border: `2px solid ${collectedFlowers[i] ? "#43a047" : "#e0e0e0"}`, fontSize: collectedFlowers[i] ? "1.3rem" : "0.8rem", transition: "all 0.3s" }}>
                    {collectedFlowers[i] || ""}
                  </div>
                ))}
              </div>
              {flowerAnim && <div className="text-center" style={{ fontSize: "2rem", animation: "flowerGrow 0.6s ease-out" }}>{currentAction.flower}</div>}
              <div className="text-center mt-1"><small style={{ color: "#2e7d32", fontWeight: 600 }}>{collectedFlowers.length} / {TOTAL_FLOWERS} وردة</small></div>
              <div className="mt-2" style={{ height: 6, background: "#f0f0f0", borderRadius: 3 }}>
                <div style={{ height: "100%", width: `${gardenProgress}%`, background: "linear-gradient(90deg, #66bb6a, #43a047)", borderRadius: 3, transition: "width 0.5s ease-out" }} />
              </div>
            </div>
            ...
            <div className="card shadow-sm border-0 p-4 mb-3" style={{ borderRadius: 16 }}>
              <div className="text-center mb-3">
                <span className="px-3 py-1 rounded-pill" style={{ background: virtue.colorLight, color: virtueColor, fontSize: "0.85rem", fontWeight: 600 }}>
                  🤔 هل هذا الفعل يدل على {virtueName}؟
                </span>
              </div>
              <p className="f-body fs-5 text-center mb-4" style={{ lineHeight: 1.8 }}>"{currentAction.text}"</p>
            ...
            {state === STATES.CORRECT && (
              <div className="card border-0 p-4 text-center anim-fade-up" style={{ borderRadius: 16, background: "#d4edda" }}>
                <div style={{ fontSize: "2.5rem" }} className="mb-2">🎉</div>
                <p className="f-display fs-5 mb-2" style={{ color: "#155724" }}>أحسنت يا {childName}!</p>
                <p className="f-body mb-3">{currentAction.isCorrect ? currentAction.feedback : currentAction.correction}</p>
                <p className="f-body mb-3" style={{ fontSize: "1.5rem" }}>{currentAction.flower} وردة جديدة!</p>
                <button onClick={handleNext} className="btn btn-lg text-white px-4" style={{ background: "#28a745", border: "none", borderRadius: 12 }}>
                  {collectedFlowers.length >= TOTAL_FLOWERS ? "🏆 شوف الحديقة!" : "➡️ التالي"}
                </button>
              </div>
            )}
            ...
            {/* Wrong */}
            {state === STATES.WRONG && (
              <div className="card border-0 p-4 text-center anim-fade-up" style={{ borderRadius: 16, background: "#fff3cd" }}>
                <div style={{ fontSize: "2.5rem" }} className="mb-2">💭</div>
                <p className="f-display fs-5 mb-2" style={{ color: "#856404" }}>فكّر مرة ثانية يا {childName}</p>
                <p className="f-body mb-3">{currentAction.isCorrect ? currentAction.feedback : currentAction.correction}</p>
                <button onClick={handleRetry} className="btn btn-lg text-dark px-4" style={{ background: "#ffc107", border: "none", borderRadius: 12 }}>
                  🔄 أحاول مرة ثانية
                </button>
              </div>
            )}
        ...
        {state === STATES.COMPLETED && (
          <div className="anim-fade-up text-center mt-4 pt-3">
            <div className="card shadow border-0 p-4 mb-4" style={{ borderRadius: 20 }}>
              ...
              <h2 className="f-display display-6 mb-2" style={{ color: "#2e7d32" }}>مبروك يا {childName}! 🎉</h2>
              <p className="f-body fs-5 mb-3">أكملت حديقة {virtueName}!</p>
              <div className="d-flex flex-wrap justify-content-center gap-2 mb-3 p-3 rounded-4" style={{ background: "#e8f5e9" }}>
                {collectedFlowers.map((flower, i) => (
                  <span key={i} style={{ fontSize: "2rem", animation: `flowerGrow 0.3s ease-out ${i * 0.1}s both` }}>{flower}</span>
                ))}
              </div>
              <div className="d-inline-block px-4 py-2 rounded-pill mb-3" style={{ background: virtue.colorLight, color: virtueColor }}>
                {virtueIcon} {virtueName} — {collectedFlowers.length} وردة
              </div>
            </div>
            <div className="d-flex flex-column gap-2">
              <button onClick={() => setState(STATES.CERTIFICATE)} className="btn btn-lg text-white px-4"
                style={{ background: "linear-gradient(135deg, #f9a825, #fbc02d)", border: "none", borderRadius: 14 }}>
                📜 شهادة الإتمام
              </button>
